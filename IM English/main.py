...
search_bar = CTkEntry(
    left_frame,
    placeholder_text="Search on products",
    font=("Arial", 24),
    justify="center",
    width=716,
)
...
total_value_label = CTkLabel(
    total_value_frame, text="Total: 0.00$", font=("Arial", 24, "bold")
)
...
def update_basket_display():
    ...
    total_value_label.configure(text=f"Total: {basket.get_total_value():.2f}$")
...
CTkLabel(
    result_frame, text="Name", anchor="w", padx=5, font=("Arial", 20, "bold"), width=435
).grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
CTkLabel(
    result_frame, text="Stock", anchor="w", padx=5, font=("Arial", 20, "bold"), width=80
).grid(row=0, column=2, sticky="nsew", padx=5, pady=5)
CTkLabel(
    result_frame, text="Price", anchor="w", padx=5, font=("Arial", 20, "bold"), width=105
...
def open_add_product_window():
    ...
    barcode_label = CTkLabel(barcode_frame, text="Barcode:", font=("Arial", 22))
    ...
    name_label = CTkLabel(name_frame, text="Name:", font=("Arial", 22))
    ...
    price_label = CTkLabel(price_frame, text="Price:", font=("Arial", 22))
    ...
    stock_label = CTkLabel(stock_frame, text="Stock:", font=("Arial", 22))
    ...
    def add_product():
        try:
            ...
            if not barcode:
                warning_label.configure(text="The barcode field cannot be empty.")
                return
            for char in barcode:
                if char not in code39_charset:
                    warning_label.configure(
                        text=f"Invalid character in the barcode: '{char}'"
                    )
                    return
            if not name:
                warning_label.configure(text="The name field cannot be empty.")
                return
            if not price:
                warning_label.configure(text="The price field cannot be empty.")
                return
            if not stock:
                warning_label.configure(text="The stock field cannot be empty.")
                return

            if storage.barcode_exists(barcode):
                warning_label.configure(text="This barcode already exists in the inventory.")
            elif storage.item_exists(name):
                warning_label.configure(text="This name already exists in the inventory.")
            else:
            ...
            warning_label.configure(
                text="You have entered invalid values. Please try again."
            )
